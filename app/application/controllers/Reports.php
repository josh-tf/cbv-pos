...
class Reports extends Secure_Controller
{
    ...
    public function graphical_summary_discounts($start_date, $end_date, $sale_type, $location_id = 'all')
    {
        ...
        $data = array(
            ...
            'xaxis_title' => $this->lang->line('reports_discount_amount'),
            'show_currency' => false,
        );
    ...
    public function get_detailed_sales_row($sale_id)
    {
        ...
        $summary_data = $this->xss_clean(array(
            ...
            'cash_cheque' => to_currency($report_data['cash_cheque']),
            'debit_credit' => to_currency($report_data['debit_credit']),
    ...
    public function detailed_sales($start_date, $end_date, $sale_type, $location_id = 'all')
    {
        ...
        foreach ($report_data['summary'] as $key => $row) {
            ...
            $summary_data[] = $this->xss_clean(array(
                ...
                'type_code' => $row['type_code'],
                'sale_date' => date($this->config->item('dateformat'), strtotime($row['sale_date'])),
                'customer_name' => ($row['customer_name'] == null ? 'No Customer' : $row['customer_name']),
                'total' => to_currency($row['total']),
                'tax' => ($row['tax'] == 0 ? '-' : to_currency_tax($row['tax'])),
                'cash_cheque' => ($row['cash_cheque'] == 0 ? '-' : to_currency($row['cash_cheque'])),
                'debit_credit' => ($row['debit_credit'] == 0 ? '-' : to_currency($row['debit_credit'])),
                'payment_type' => $row['payment_type'],
